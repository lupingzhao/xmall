<template>
  <!-- 所有精选 -->
  <div>
    <div v-for="(item, index) in allselect1" :key="index">
      <div class="w1">
        <div class="w head">{{ allselect1[index].name }}</div>
        <div class="w bigbox">
          <div
            v-for="(item1, index1) in allselect1[index].panelContents"
            :key="index1"
            class="box"
            :class="{ 'w-5': index1 === 0 }"
            @click.stop="view(item1.productId, item1)"
          >
            <img :src="item1.picUrl" alt="" class="img" />
            <div
              style="text-align: center; margin-bottom: 10px"
              v-if="index1 !== 0"
            >
              {{ item1.productName }}
            </div>
            <div
              style="text-align: center; margin-bottom: 10px"
              v-if="index !== 0"
            >
              {{ item1.subTitle }}
            </div>
            <div
              style="text-align: center; line-height: 50px; color: red"
              class="price"
              v-if="index1 !== 0"
            >
              ￥{{ item1.salePrice }}
            </div>
            <div
              style="text-align: center; height: 50px"
              class="tip"
              v-if="index1 !== 0"
            >
              <el-button type="info" @click="view(item1.productId, item1)"
                >查看详情</el-button
              >
              <!-- 阻止父元素的点击事件 -->
              <el-button type="primary" @click.stop="add(item1.productId)"
                >添加至购物车</el-button
              >
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import axios from "axios";
export default {
  name: "",
  props: {
    allselect1: {
      type: Array,
    },
  },
  data() {
    return {};
  },
  components: {},
  methods: {
    // 购物车添加数据
    add(a) {
      console.log(a);
      axios
        .post("/api/goods/addCart", { productId: a })
        .then((res) => {
          // console.log(res.data);
          this.$parent.$parent.cars();
          // 第一级是home 第二级是layout  调用layout里面的数据
        })
        .catch((err) => {
          console.log(err);
        });
    },
    view(a, b) {
      // 商品id
      // console.log(a);
      //dang当前点击的对象
      // console.log(b);
      // 跳转页面时新开一个窗口
      let routeData = this.$router.resolve({
        path: "/goodsDetail",
        query: { productId: a },
      });
      window.open(routeData.href, "_blank");

      // this.$router.push({
      //   // 传递当前点击的文章标题的id 在接受页用id发请求
      //   path: "/goodsDetail",
      //   query: {
      //     productId: a,
      //   },
      // });
    },
  },
  mounted() {
    // console.log(this.allselect1);
  },
  computed: {},
  watch: {},
};
</script>

<style lang='scss' scoped>
.head {
  padding: 20px 0;
  margin: 20px auto 0;
  background-color: rgb(243, 243, 243);
  color: rgb(39, 38, 38);
}
.bigbox {
  overflow: hidden;
  flex-wrap: wrap;
  align-content: flex-start;
  background-color: #fff;
  .box {
    width: 25%;
    height: 400px;
    background-color: #fff;
    margin-bottom: 1px;
    .img {
      width: 100%;
    }
    .tip {
      display: none;
    }
  }
  .box:hover {
    .tip {
      margin-top: 20px;
      display: block;
    }
    .price {
      display: none;
    }
    //  move 2s linear infinite
    animation: dh 1.5s;
    animation-fill-mode: forwards;
    // 停留在最后一帧
  }
  @keyframes dh {
    from {
      box-shadow: 0 0 4px rgba(95, 94, 94);
      transform: translateY(0);
      // height: 500px;
    }

    to {
      // height: 495px;
      transform: translateY(-4px);
      box-shadow: 0 0 20px rgba(95, 94, 94);
    }
  }
  .w-5 {
    width: 50%;
    img {
      height: 100%;
    }
    &:hover {
      img {
        opacity: 0.8;
      }
      box-shadow: inset 0 0 5px gray !important;
      animation: none !important;
    }
  }
}
</style>